...
# **get_newsletters**
> InlineResponse200 get_newsletters(page=page, per_page=per_page)
...
# **send_newsletter**
> Newsletter send_newsletter(config=config)
...
try:
    # Send a newsletter.
    api_response = api_instance.send_newsletter(config=config)
    pprint(api_response)
except ApiException as e:
